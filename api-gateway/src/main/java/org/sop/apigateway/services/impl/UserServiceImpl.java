...
public class UserServiceImpl implements UserService {
    ...
    public String getUsernameById(Long userId) {
        return userRepository.findById(userId)
                .map(User::getUsername) // Get the username from the User entity
                .orElse(null); // Return null or handle the case where the user is not found
    }
    ...
    public User update(User user) {
        ...
        if (foundUser == null) return null; // Handle user not found
        if ((!foundUser.getUsername().equals(user.getUsername()) && userRepository.existsByUsername(user.getUsername())) || 
            (!foundUser.getEmail().equals(user.getEmail()) && userRepository.existsByEmail(user.getEmail())))
            return null; // Handle duplicate username/email
    ...
    public boolean addFriend(Long id1, Long id2) {
        ...
        if (user1 == null || user2 == null) return false; // User not found
